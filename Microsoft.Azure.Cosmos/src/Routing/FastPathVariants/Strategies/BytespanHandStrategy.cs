...
namespace Microsoft.Azure.Cosmos.Routing.FastPathVariants.Strategies
{
    ...
    /// using a hand-rolled big-endian ulong compare per probe (variant E).
    /// </summary>
    internal sealed class BytespanHandStrategy : IRoutingFastPathStrategy
    {
        private readonly byte[] sortedByteBoundaries;
        private readonly PartitionKeyRange[] orderedRanges;

        internal BytespanHandStrategy(IReadOnlyList<PartitionKeyRange> orderedRanges)
        {
        ...
        public PartitionKeyRange Resolve(string effectivePartitionKey)
        {
            ...
            {
                throw new ArgumentException("EPK is not 32-char hex; BytespanHandStrategy is V2-hash-only.", nameof(effectivePartitionKey));
            }
